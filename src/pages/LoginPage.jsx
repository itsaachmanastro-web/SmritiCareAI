...
export default function LoginPage() {
  ...
  // Navigate when user is authenticated (e.g. returning from Google OAuth or active session)
  useEffect(() => {
    if (currentUser) {
      const targetRole = currentUser.role || activeRole;
      if (targetRole === 'healthcare') {
  ...
  // Optional multi-screen reference view toggle
  ...
  // Form Submit for Caregiver / Clinician
  ...
  const handleQuickDemo = async (role) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        else if (role === 'healthcare') navigate('/clinician/dashboard');
        else navigate('/caregiver/dashboard');
      }, 350);
    } catch (err) {
      setIsLoading(false);
      setErrorMsg('Could not log into demo.');
    }
  };
  ...
  const handleGoogleSignIn = async (roleKey) => {
    ...
    try {
      ...
      if (res?.success) {
        ...
        playMatchSuccessSound();
        setTimeout(() => {
          if (roleKey === 'patient') navigate('/patient/home');
          else if (roleKey === 'healthcare') navigate('/clinician/dashboard');
          else navigate('/caregiver/dashboard');
        }, 350);
      } else {
  ...
  const handleSignUpSubmit = async (e) => {
    ...
    if (res.success) {
      ...
      playMatchSuccessSound();
      setTimeout(() => {
        if (activeRole === 'healthcare') navigate('/clinician/dashboard');
  ...
  const roleConfigs = {
    patient: {
      ...
      subtitle: t('auth.pinPrompt') || 'Enter your 4-digit PIN to continue.',
  ...
  // Single Role Split-Screen Component with Viewport-Fit (No Scrolling on 1440x900)
  const renderAuthScreen = (config, isPreview = false) => {
    ...
    return (
      <div className={`w-full h-full flex flex-col lg:flex-row bg-[#080D11] text-slate-100 ${isPreview ? 'rounded-2xl overflow-hidden border border-white/15 shadow-2xl scale-100 min-h-[580px]' : 'lg:h-full lg:overflow-hidden'}`}>
        ...
        {/* Reduced image container height & aspect ratio with generous breathing room, NO duplicate logo */}
        <div className="w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-6 lg:p-8 shrink-0">
          <div className="relative w-full max-w-lg h-[340px] sm:h-[400px] lg:h-[480px] xl:h-[520px] max-h-[calc(100vh-6.5rem)] rounded-2xl overflow-hidden border border-white/10 shadow-2xl flex flex-col justify-between p-6 sm:p-8">
            {/* Authentic Photograph with controlled object-cover */}
            <div 
            ...
            {/* Top Category Label (NO DUPLICATE SMRITICARE LOGO) */}
        ...
        {/* Calibrated vertical rhythm fitting comfortably in 900px viewport without scrolling */}
        <div className="w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-6 lg:p-8 shrink-0">
          <div className="w-full max-w-md flex flex-col justify-center space-y-3 sm:space-y-4">
            ...
            <div className="flex items-center justify-between pb-2 border-b border-white/5">
              ...
              <button
                type="button"
                onClick={() => alert('For support, email support@smriticare.org or use the 1-tap demo below.')}
                className="inline-flex items-center gap-1.5 text-xs text-slate-400 hover:text-white transition-colors cursor-pointer"
              >
            ...
            {/* Error Message Alert */}
            {errorMsg && (
              <div className="p-2.5 rounded-lg bg-rose-950/60 border border-rose-800 text-rose-200 text-xs flex items-center gap-2">
                <AlertCircle className="w-3.5 h-3.5 text-rose-400 shrink-0" />
                <span>{errorMsg}</span>
              </div>
            )}

            {/* Form Area: PATIENT PIN vs CAREGIVER/CLINICIAN FORM */}
            <div>
              {isPatient ? (
                /* SCREEN 1: SENIOR / PATIENT (PIN Keypad Mode) */
                <div className="space-y-3.5">
                  ...
                  {/* Compact & Accessible Numeric Keypad */}
                ...
                /* SCREEN 2 & 3: CAREGIVER & HEALTHCARE PROFESSIONAL FORMS */
                <form onSubmit={handleSignIn} className="space-y-3">
                  {/* Field 1: Email or Phone */}
                  <div>
                    <label className="block text-xs text-slate-300 font-medium mb-1">
                      {isCaregiver ? (t('auth.email') || 'Email Address') : (t('auth.professionalEmail') || 'Professional Email')}
                    </label>
                    <div className="relative">
                      ...
                      <input
                        type="email"
                        required
                        placeholder={isCaregiver ? 'name@example.com' : 'doctor@hospital.org'}
                        value={email}
                  ...
                  {googleError && (
                    <div className="p-2.5 rounded-lg bg-rose-950/50 border border-rose-800/60 text-[11px] text-rose-300 leading-snug text-left">
                      {googleError}
                    </div>
                  )}
                  ...
                  >
                    ...
                    <span>{googleLoading ? 'Connecting to Google...' : (t('auth.continueWithGoogle') || 'Continue with Google')}</span>
                  </button>

                  {/* Footer Register / Admin Link */}
                  <div className="text-center pt-0.5">
                    {isCaregiver ? (
                      <p className="text-[11px] text-slate-400">
                        {t('auth.newHere') || 'New here?'}{' '}
  ...
  return (
    <div className="min-h-screen lg:h-screen lg:max-h-screen bg-[#070B0E] text-slate-100 flex flex-col justify-between relative selection:bg-emerald-500/20 selection:text-emerald-300 font-sans overflow-x-hidden lg:overflow-hidden">
      ...
      <header className="w-full h-14 border-b border-white/10 backdrop-blur-md bg-[#070B0E]/90 px-6 lg:px-12 flex items-center justify-between z-30 shrink-0">
        {/* Brand Logo & Title — ONLY SMRITICARE LOGO ON THE PAGE */}
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Language Selector Dropdown with all 5 languages */}
      ...
      {/* 2. MAIN VIEWPORT-BOUND CONTENT VIEW (100vh Layout, No Unnecessary Page Scroll) */}
      <main className="flex-1 min-h-0 w-full h-[calc(100vh-3.5rem)] flex flex-col justify-center overflow-y-auto lg:overflow-hidden bg-[#070B0E]">
        {showComparisonView ? (
          ...
          /* DEFAULT FOCUSED MODE: 100vh Desktop Viewport-Bound Screen with NO SCROLLING */
            ...
            <form onSubmit={handleSendReset} className="space-y-3">
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                <button
                  ...
                  className="px-4 py-2 rounded-lg text-xs text-slate-300 hover:text-white"
                >
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg text-xs font-semibold bg-white text-slate-950 hover:bg-slate-200"
                >
