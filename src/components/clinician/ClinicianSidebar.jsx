...
export default function ClinicianSidebar({
  ...
  return (
    <aside
      aria-label="Clinician Sidebar Navigation"
    ...
    >
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 text-left cursor-pointer ${
                  isActive
                    ? 'bg-[#DDF3E8] text-[#143D30] font-semibold shadow-xs dark:bg-[#122C27] dark:border dark:border-[#1E4D43] dark:text-[#4ADE80]'
                    : 'text-[#4A5954] dark:text-[#94A3B8] hover:text-[#192320] dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5'
                }`}
              >
                <Icon className={`w-4 h-4 shrink-0 ${isActive ? 'text-[#143D30] dark:text-[#4ADE80]' : 'text-[#6E7D76] dark:text-[#64748B]'}`} />
      ...
      {/* 3. BOTTOM SECTION: CLINICIAN PROFILE CARD */}
      <div className="pt-4 border-t border-[#ECE7DE] dark:border-[#152225]">
        <button
          type="button"
        ...
        >
          <UserAvatar
            user={currentUser}
            image={currentUser?.profileImage || currentUser?.avatar || '/assets/images/clinician-doctor-avatar.jpg'}
            size="sm"
